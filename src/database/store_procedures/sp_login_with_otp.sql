-- Stored Procedure: sp_login_with_otp
-- Purpose: Login user with verified OTP and return user details
-- Returns: master_user_id, business_id, branch_id, role_id, is_owner, and user details
...
BEGIN
    ...
    SELECT master_otp_status_id
      INTO v_verified_otp_status_id
      FROM master_otp_status
     WHERE code = 'VERIFIED' AND is_deleted = 0
     LIMIT 1;
    
    -- Get LOGIN OTP type ID
    ...
    IF v_ok = 1 THEN
        SELECT id, otp_code_hash, attempts
         ...
         WHERE target_identifier = p_email
           AND otp_status_id = v_verified_otp_status_id
           AND expires_at > NOW()
        ...
        IF v_otp_record_id IS NULL THEN
            ...
            SET v_ok = 0;
        ELSE
            -- OTP is already verified, proceed to fetch user
            SET v_verified_otp_status_id = 1; -- Mark that we have verified OTP
        END IF;
    END IF;
